...
* Controlled Reception Pattern Antenna / CRPA (adaptive antenna array that nulls jammers), 受控接收模式天线, Антенна с управляемой диаграммой направленности
* Anti-jam antenna / adaptive nulling (spatial-domain interference suppression), 抗干扰天线, Антипомеховая антенна
* Adaptive beamforming for GNSS (digital phased-array signal processing), 自适应波束成形, Адаптивное формирование луча
* GNSS interference mitigation (filtering, excision, and spatial processing), 全球导航卫星系统干扰抑制
* Navigation warfare / NAVWAR (protect, prevent, preserve PNT), 导航战, Навигационная война
* PNT resilience & alternative navigation (assured timing in GPS-denied conditions), 定位导航授时弹性
""")
...
def load_centroids_asat():
  ...
  #  df = pd.read_json(df.to_json())
    dg = pd.read_pickle('jammingcrpacentroids2d.pkl.gz')
    #return dg
    return dg[dg.cluster != -1]

@st.cache_data()
def load_dftriple_asat():
    dg = pd.read_pickle('jammingcrpadftriple2d.pkl.gz')
    return dg

@st.cache_data()
def load_dfinfo_asat():
    dg = pd.read_pickle('jammingcrpadfinfo2d.pkl.gz')
    #return dg
    return dg[dg['cluster'] != -1]
